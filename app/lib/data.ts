...
export async function fetchFilteredCustomersPaged(
  query: string,
  currentPage: number,
) {
  ...
  try {
    ...
    console.error('Database Error:', err);
    throw new Error('Failed to fetch paginated customer table.');
  }
}
...
export async function fetchCustomersPages(query: string) {
  try {
    ...
    const totalPages = Math.max(1, Math.ceil(Number(data[0].count) / CUSTOMERS_PER_PAGE));
    return totalPages;
  } catch (err) {
    console.error('Database Error:', err);
